...
| VdoCipher | VDOCIPHER_PLAYBACK_WHITELIST_HREF | operational config | optional | optional | src/lib/vdocipher-playback.ts | Optional VdoCipher OTP `whitelisthref` override. When unset, playback OTPs use the active request hostname, then fall back to the hostname from `NEXTAUTH_URL`; production can set `elearning.vienphuongnam.com.vn` or a VdoCipher regex group such as `(elearning.vienphuongnam.com.vn|vienphuongnam.com.vn)`. |
...
| VdoCipher | Playback URL whitelist | `VDOCIPHER_PLAYBACK_WHITELIST_HREF=<STAGING_HOSTNAME>` or ensure the active request hostname is the domain allowed in each VdoCipher account dashboard; `NEXTAUTH_URL` is only the fallback. |
