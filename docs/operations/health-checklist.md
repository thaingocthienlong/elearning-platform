...
| HEALTH-SCRIPT-04 | Tencent setup validation | `npm run verify:tencent -- --strict` | not run | |
...
| HEALTH-TENCENT-01 | Tencent DRM | Can an entitled staging learner obtain Tencent playback session data and a license for a test video? | not run | |
| HEALTH-TENCENT-02 | Tencent webhooks | Is `<STAGING_ORIGIN>/api/webhook/tencent` configured and signature-verified? | not run | |
