...
| Tencent VOD Commercial DRM | Configure `TENCENT_SECRET_ID`, `TENCENT_SECRET_KEY`, `TENCENT_VOD_REGION`, `TENCENT_VOD_PROCEDURE_NAME`, license URLs, and SimpleAES/basic HLS Apple fallback output when FairPlay is unavailable. | Upload and process a staging video, then confirm token/license/fallback requests are limited to authorized playback. |
