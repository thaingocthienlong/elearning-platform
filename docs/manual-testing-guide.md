...
2. Sign in as an entitled learner.
3. Open `/watch/<videoId>`.
4. Accept IPR consent and start playback.
5. Sign in as a learner without access and request the same watch page or `/api/vdocipher/otp`.
...
1. Configure `VDOCIPHER_ACCOUNT_IDS`, `VDOCIPHER_DEFAULT_ACCOUNT_ID`, and each `VDOCIPHER_API_SECRET_<ACCOUNT>`.
